...
def parse_event_duration(duration_str: str) -> int:
    try:
        ...
        raise ValueError(f"Ошибка парсинга времени: {str(e)}")        "cost_multiplier": 2.2
    },
    "ruby_banana": {
        "name": "❤️ Рубиновый Банан",
        "base_cost": 6000, 
...
def parse_event_duration(duration_str: str) -> int:
    try:
        ...
        raise ValueError(f"Ошибка парсинга времени: {str(e)}")    return (user.get("bananas", 0) or 0) >= price
