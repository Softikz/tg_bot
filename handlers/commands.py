...
def admin_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="⏸️ Технический перерыв", callback_data="admin_pause")],
...
async def pause_bot(message: types.Message, state: FSMContext):
    """Поставить бота на паузу (только для админа)"""
    ...
    await message.answer(
        "⏸️ <b>Технический перерыв</b>\n\n"
        "Введите сообщение для пользователей о техническом перерыве:\n\n"
        "Пример: <i>Идёт технический перерыв, примерное время ≈ 2 часа</i>",
        parse_mode="HTML"
    )
    await state.set_state(AdminStates.waiting_for_pause_message)

@router.message(AdminStates.waiting_for_pause_message)
async def process_pause_message(message: types.Message, state: FSMContext):
    """Обрабатывает сообщение о паузе"""
    pause_message = message.text
    
    # Устанавливаем паузу
    db.set_bot_pause(True, pause_message)
    
    # Уведомляем админа
    await message.answer(
        f"✅ <b>Бот поставлен на паузу!</b>\n\n"
        f"📝 Сообщение для пользователей:\n"
        f"<i>{pause_message}</i>\n\n"
        f"Для возобновления работы используйте /startbotwork",
        parse_mode="HTML"
    )
    ...
    await message.answer(f"📨 Уведомлено пользователей: {notified}")
    await state.clear()

@router.message(Command("startbotwork"))
async def start_bot_work(message: types.Message):
    """Возобновить работу бота (только для админа)"""
    user_id = message.from_user.id
    ...
    # Снимаем паузу
    db.set_bot_pause(False)
    
    # Уведомляем админа
    await message.answer(
        "✅ <b>Работа бота возобновлена!</b>\n\n"
        "Все пользователи снова могут использовать бота.",
        parse_mode="HTML"
    )
    ...
    notified = await send_notification_to_all_users(
        ...
        "Бот снова доступен для использования! 🍌"
    )
...
async def start_command(message: types.Message, state: FSMContext):
    ...
    # Проверяем паузу бота (админ всегда имеет доступ)
    if is_bot_paused() and not is_admin(user_id):
        pause_msg = get_pause_message()
        await message.answer(
            f"⏸️ <b>{pause_msg}</b>\n\n"
            f"Приносим извинения за временные неудобства!",
            parse_mode="HTML"
...
async def start_registration(callback: CallbackQuery, state: FSMContext):
    ...
    if is_bot_paused() and not is_admin(callback.from_user.id):
        await callback.answer("⏸️ Бот на техническом перерыве!", show_alert=True)
        return
...
async def handle_admin_commands(callback: CallbackQuery, state: FSMContext):
    ...
    if action == "admin_stats":
        ...
        # Статус паузы
        pause_status = "⏸️ НА ПАУЗЕ" if db.is_bot_paused() else "✅ АКТИВЕН"
        pause_message = db.get_pause_message()
        ...
        if db.is_bot_paused():
            stats_text += f"📝 Сообщение: {pause_message}\n"
            
        stats_text += f"🕒 Время сервера: {time.strftime('%Y-%m-%d %H:%M:%S')}"
        ...
        # Кнопка технического перерыва в админ-панели
        if db.is_bot_paused():
            # Если уже на паузе, предлагаем возобновить
            keyboard = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="▶️ Возобновить работу", callback_data="admin_resume")],
                [InlineKeyboardButton(text="✏️ Изменить сообщение", callback_data="admin_change_pause_message")],
                [InlineKeyboardButton(text="⬅ Назад", callback_data="admin_back")]
            ])
            current_message = db.get_pause_message()
            await callback.message.edit_text(
                f"⏸️ <b>Бот сейчас на паузе</b>\n\n"
                f"📝 Текущее сообщение:\n"
            ...
            # Если не на паузе, предлагаем поставить на паузу
            keyboard = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="⏸️ Поставить на паузу", callback_data="admin_start_pause")],
                [InlineKeyboardButton(text="⬅ Назад", callback_data="admin_back")]
            ])
            await callback.message.edit_text(
                "⏸️ <b>Технический перерыв</b>\n\n"
                "Вы можете временно остановить работу бота для всех пользователей, кроме администратора.\n\n"
                "Выберите действие:",
                parse_mode="HTML",
                reply_markup=keyboard
            )
        ...
        # Возобновление работы из админ-панели
        db.set_bot_pause(False)
        ...
        await callback.message.edit_text(
            f"✅ <b>Работа бота возобновлена!</b>\n\n"
    ...
    elif action == "admin_change_pause_message":
        await callback.message.edit_text(
            "✏️ <b>Изменение сообщения о перерыве</b>\n\n"
...
async def process_admin_event_duration(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer(f"❌ {str(e)}\n\nПопробуйте еще раз в формате 'часы:минуты':")db = DB()
ADMIN_PASSWORD = "sm10082x3%"
ADMIN_ID = 5748972158
log = logging.getLogger(__name__)
