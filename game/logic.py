...
def has_active_gold(user: Dict) -> bool:
    gold_expires = user.get("gold_expires", 0)
    return gold_expires > time.time()

def has_active_event(user: Dict) -> bool:
    event_expires = user.get("event_expires", 0)
    return event_expires > time.time()
...
def effective_per_click(user: Dict) -> int:
    ...
    rebirth_multiplier = 1.0 + (user.get("rebirths", 0) * 0.5)
    
    total_multiplier = gold_multiplier(user) * event_multiplier * rebirth_multiplier
    return int(base_click * total_multiplier)
